	...
	function __socket__ {
		if [[ ! -f "__lol__tor.conf" ]]; then
			...
			if ((app == 1)); then {
				...
				tor -f __lol__tor.conf 2>/dev/null 1>/dev/null &
			}; fi
		};
      }; __socket__ "$app" "$2";
}
...
def: system.setup(){
	if ! (command -v tor &> /dev/null); then
		...
		if [[ -z "$mode" ]]; then
			...
			fi; if ! (curl --socks5-hostname 127.0.0.1:9050 google.com &> /dev/null); then sleep 22s; fi; if ! (curl --socks5-hostname 127.0.0.1:9050 google.com &> /dev/null); then println_info " Failed connection in torsocks"; exit 5; fi
};

shopt -s expand_aliases

IO.AS "system.setup" to "my.__tor__:";
IO.AS 'if ! (curl --socks5-hostname 127.0.0.1:9050 --url google.com &> /dev/null); then println_info " Failed connection in torsocks how to use its -> my.__tor__: default; curlTor [METHODS] [URL] OPTIONS --help"; exit 23; fi; killall -HUP tor; curl --socks5-hostname 127.0.0.1:9050 --insecure --compressed' to "curlTor";
